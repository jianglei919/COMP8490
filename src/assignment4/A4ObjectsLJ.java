...
public abstract class A4ObjectsLJ {
...
/* ===== Base ===== */
class BaseShapeA extends A4ObjectsLJ {
    public BaseShapeA() {
...
/* ===== 文字 ===== */
class ColorString extends A4ObjectsLJ {
    String str;
    Color3f clr;
    float scl;
    Vector3f pos;
 ...
 * 支撑整个风扇的底座支架
 */
class StandObjectA extends A4ObjectsLJ {
    public StandObjectA() {
        scale = 1.0;
 ...
 * 电源开关
 */
class SwitchObjectA extends A4ObjectsLJ {
...
/* ===== Shaft（圆柱立柱） ===== */
class ShaftObjectLJ extends A4ObjectsLJ {
 ...
 * 电机外壳
 */
class MotorObjectLJ extends A4ObjectsLJ {
 ...
 * 外部防护罩（圆形网格）
 */
class GuardObjectLJ extends A4ObjectsLJ {
 ...
 * 风扇叶片（通常有 3~4 片）
 */
class BladeObjectLJ extends A4ObjectsLJ {
    public BladeObjectLJ(double scaleFactor) {
        ...
        // 1) 尝试用名字
        BranchGroup bladesByName = A4ObjectsLJ.extractByName(objBG, "blade", "blades", "leaf", "prop");
        Node use = (bladesByName.numChildren() > 0)
                ? bladesByName
                : A4ObjectsLJ.keepThinPlates(A4ObjectsLJ.stripLines(objBG), 0.12 /*薄*/, 0.35 /*够长*/);
        ...
        A4ObjectsLJ.applyAppearance(use, app);
    }
