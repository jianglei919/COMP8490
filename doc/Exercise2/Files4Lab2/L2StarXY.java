...
class L2StarXY extends GroupObjects {
	...
	private static LineArray one_Star(int num, Color3f clr, float r) {
		...
		for (int i = 0; i <  num; i++) {                   // define lines for the start
			...
			lineArr.setColor(i * 2 + 1, CommonsXY.Green);  // set color for end points
		}
	...
	public static Shape3D line_Shape(float r) {
		...
		LineArray starGeometry = one_Star(n, CommonsXY.Red, r); 

		return new Shape3D(starGeometry);
	}
	
	public L2StarXY(float r) {
		super(line_Shape(r));
	}
}
